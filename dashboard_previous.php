...
$notifications = [];
$sql_notif = "SELECT rb.*, cr.room_num, ch.start_time as time, ch.end_time, p.description as priority_name FROM Room_Booking rb 
              JOIN Checking ch ON rb.checking_id = ch.id 
              JOIN Class_room cr ON ch.class_room_id = cr.id 
              JOIN priority p ON rb.priority_id = p.priority_id 
              WHERE rb.user_id = ? ORDER BY ch.start_time DESC LIMIT 10";
    ...
    <style>
        ...
        * {
        ...
        .sidebar {
            position: fixed;
            right: -320px;
            /* Increased slightly for shadow clearance */
            top: 0;
            height: 100%;
            width: 300px;
                ...
                ?>
                    <div class="notification-item">
                        ...
                        <div class="notification-item-content">
                            ...
                            <small><?= date('M d, Y h:i A', strtotime($notif['time'])) ?> - <?= date('h:i A', strtotime($notif['end_time'])) ?> | Status: <?= ucfirst($notif['status']) ?><?= $cancel_msg ?></small>
                        </div>
                    </div>
                <?php endforeach; ?>
            <?php endif; ?>
